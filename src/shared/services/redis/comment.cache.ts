...
export class CommentCache extends BaseCache {
  ...
  public async savePostCommentToCache(postId: string, value: string): Promise<void> {
    try {
      ...
      const commentsCount: string[] = await this.client.HMGET(`post:${postId}`, 'postsCount')
  ...
  public async getCommentsFromCache(postId: string): Promise<ICommentDocument[]> {
    try {
      ...
      const reply: string[] = await this.client.LRANGE(`comments:${postId}`, 0, -1) // get all items inside redis list
