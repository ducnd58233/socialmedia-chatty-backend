...
class CommentService {
  public async addCommentToDB(commentData: ICommentJob): Promise<void> {
    ...
    if (response[2].notifications.comments && userFrom !== userTo) {
      ...
      const notifications = await notificationModel.insertNotification({
        userFrom,
        userTo,
        message: `${username} commented on your post. ${response[0].comment}`,
  ...
  public async getPostCommentsNamesFromDB(
    query: IQueryComment,
    ...
    const commentsNamesList: ICommentNameList[] = await CommentsModel.aggregate([
      { $match: query },
      { $sort: sort },
      { $group: { _id: null, names: { $addToSet: '$username' }, count: { $sum: 1 } } }, // seperate data into group keys
      { $project: { _id: 0 } } // exclude the id from final result
    ])
